 ...
 */
class XenoApplication : Application(), ImageLoaderFactory {
	...
	override fun onConfigurationChanged(newConfig: android.content.res.Configuration) {
		...
		com.xeno.i18n.I18n.refreshSystemLanguage(this)
	}
			...
			Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
				runCatching {
					File(logDir, "crash-${System.currentTimeMillis()}.txt").writeText(
						buildString {
							appendLine("XENO crash log")
							appendLine("time=${System.currentTimeMillis()}")
							appendLine("thread=${thread.name}")
							appendLine(
								"game=" + runCatching {
									com.xeno.runtime.MainActivityRuntime.currentGame.value
