...
object Ps3PatchRepo {
    ...
    fun download(): Result {
        ...
        val res = runCatching {
            com.xeno.HttpClient.doRequest(patchUrl(engineVersion), userAgent = "XENO")
    ...
    private const val PREFS_NAME = "XENO"
    ...
    fun ensureBundledPatches(context: Context) {
        ...
        if (yaml.isNullOrBlank()) {
            android.util.Log.e("XENO", "canary patches: $BUNDLED_ASSET missing from assets")
            return
        }
        ...
        if (imported < 0) {
            android.util.Log.e("XENO", "canary patches: import failed")
            return
        }
        ...
        val allEnabled = pending.all { b ->
            ...
            if (!ok) {
                android.util.Log.e("XENO", "canary patches: could not enable ${b.name} (${b.hash})")
            }
            ok
        }
        ...
        if (allEnabled) {
            prefs.edit().putInt(KEY_BUNDLED_REVISION, BUNDLED_REVISION).apply()
            android.util.Log.i("XENO", "canary patches: imported $imported, enabled ${pending.size}")
        }
    }
}
