...
object ControllerMappings {
	...
	private class PerStickPref(val baseKey: String, val def: Float, val lo: Float, val hi: Float) {
		...
		fun set(left: Boolean, v: Float) {
			...
			val serial = if (com.xeno.ui.InGameOverlay.settingsScope.value ==
					com.xeno.config.SettingsScope.Game) runtimeSerial() else null
			MainActivityRuntime.prefs.edit {
    ...
    fun setRumbleEnabled(on: Boolean) {
        ...
        com.xeno.NativeApp.sRumbleEnabled = on
    }
    ...
    fun setPhoneRumbleEnabled(on: Boolean) {
        ...
        com.xeno.NativeApp.sPhoneRumbleEnabled = on
    }
    ...
    // A CUSTOM stick direction can fire an XENO hotkey (Quick Save/Load State, etc.)
     ...
     *  the freed physical button can instead be assigned as an XENO hotkey. With a
    ...
    enum class SysHotkey(val prefKey: String, val label: String, val supported: Boolean = true) {
        ...
        // emulated USB keyboard (com.xeno.input.SoftKeyboard). A hotkey rather than a setting
    ...
    fun warmRuntimeCaches() {
        runtimeBindings()
        com.xeno.ui.touch.TouchControls.warmRuntimeMacroCache()
    }
    ...
    fun installRuntimeCacheInvalidation() {
        ...
        val listener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            ...
            if (changedKey.startsWith("touch.macro."))
                com.xeno.ui.touch.TouchControls.invalidateRuntimeMacroCache()
        }
