...
object ConfigDatabase {
    ...
    private val UNSAFE_ON_ANDROID = setOf(
        ...
        // number of hardware threads". Every XENO device has 8 cores -- armv8.1-a is the compile
    ...
    /**
     * XENO's own per-title settings, layered on top of whatever the RPCS3 database says.
     *
    ...
    fun setEnabled(enabled: Boolean) {
        ...
        // Ours are XENO fixes, not part of the downloaded database, so they stay live
    ...
    fun refresh(): Int {
        val body = runCatching {
            val connection = (URL(URL_V1).openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("User-Agent", "XENO")
            }
            try {
