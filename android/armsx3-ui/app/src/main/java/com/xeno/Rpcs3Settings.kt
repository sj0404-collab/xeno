...
/**
 * XENO UI setting -> RPCS3 config node.
 *
...
object Rpcs3Settings {
     ...
     *
     * The UI sends XENO's own lowercase ids ("vulkan", "opengl", "auto",
    ...
    /**
     * XENO sends an upscale MULTIPLIER (1.0 = native). RPCS3 wants a
    ...
    /**
     * XENO's Display FPS Cap slider is free-form (0..60, any value), which the
    ...
    // ---- Performance overlay --------------------------------------------
    //
    // XENO has a boolean per element (osdShowFPS, osdShowCPU, ...). RPCS3
     ...
     * are REJECTED, not clamped, so the clamp has to match exactly. XENO's
    ...
    /** Emulated clock speed as a percentage, 10..3000. XENO calls this nominal speed. */
