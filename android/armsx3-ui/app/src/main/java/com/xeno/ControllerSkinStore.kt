...
object ControllerSkinStore {
    ...
    private const val KEY_MIGRATED_DEFAULT = "skin.migrated.xenodefault"
     ...
     *   NONE   the XENO row was chosen, meaning the built-in drawables, not the default
     ...
     * choosing the XENO row after this runs sticks rather than being undone on next launch.
     */
     ...
     *  credit, while the XENO and ARMSX1 packs were made for us and should say so. */
    ...
    val BUILTIN: List<BuiltinSkin> = listOf(
        BuiltinSkin("builtin:xeno_textured", "XENO Textured", "controller_skins/xeno_textured", "bagas"),
     ...
     *
     * Null used to mean the built-in XENO drawables and was also the default, so the two were
     * the same thing and nothing had to say which. They are separate now: XENO Textured is the
     * default, XENO is still reachable as its own choice, and null still means the drawables.
     ...
     */
    const val DEFAULT_SKIN_ID = "builtin:xeno_textured"
    ...
    fun setActive(ctx: Context, id: String?, serial: String? = null) {
        ...
        MainActivityRuntime.prefs.edit().apply {
            ...
            // mean "never chose", and picking XENO would silently hand back XENO
     ...
     *
     * The built-in XENO look has no asset folder, so its strip is a drawable instead and
     ...
     *  download path ([com.xeno.SkinRepo]).
     *
