...
import java.io.File
import com.xeno.input.PadRouter

/**
 * Translation layer between XENO's [NativeApp] surface and RPCS3.
 *
 * NativeApp exists to keep XENO's 129-file UI compiling unchanged. This is
 * where the two models are actually reconciled, and they differ in ways that
 * matter:
 *
 *  - PAD INPUT. XENO sends per-button edges (setPadButton(index, range,
 ...
 *
 *  - SETTINGS. XENO addresses a PCSX2 INI by (section, key). RPCS3 has a
...
object Rpcs3Bridge {
    ...
    /**
     * Why this CPU cannot run XENO, or null if it can.
     *
     ...
     * running libxeno-core.so's initialisers, which means it happens before any of our code can
    ...
    private fun unsupportedCpuReason(): String? {
        ...
        return "This device's CPU is ARMv8.0 and XENO requires ARMv8.1 " +
            "(missing " + names + "). No XENO build can run on it."
    }
    ...
    fun initializeOnce(context: Context) {
        ...
        if (RPCSX.activeLibrary.value == null) {
            ...
            unsupportedCpuReason()?.let { reason ->
                ...
                android.util.Log.e("XENO", reason)
                reportFatal(context, reason)
                return
            }

            // The APK no longer ships libxeno-core.so (Winlator-style: smaller
            ...
            // the downloaded file.
            val nativeCore = File(libDir, "libxeno-core.so")
            ...
            if (core == null) {
                android.util.Log.e("XENO", "core missing: checked ${'$'}{nativeCore.absolutePath} and ${'$'}{downloadedCore.absolutePath}")
                return
            }
            if (!RPCSX.openLibrary(core.absolutePath)) {
                android.util.Log.e("XENO", "core failed to load: ${'$'}{core.absolutePath}")
                return
            }
        }
        ...
        // that: XENO's UI has no PS2-style "hand the core a BIOS folder" step,
        ...
        val root = runCatching {
            com.xeno.runtime.MainActivityRuntime.assetCopyRoot(context)
    ...
    private fun applyFirmwareRedirect() {
        ...
        try {
            ...
            android.util.Log.i("XENO", "firmware redirect -> $extDir")
        } catch (e: Exception) {
            android.util.Log.e("XENO", "failed to write vfs.yml for firmware redirect", e)
        }
    }
    ...
    fun initialize(rootPath: String) {
        ...
        runCatching { com.xeno.config.ConfigDatabase.purgeIfStale() }
        // Applies whether or not the database was ever downloaded.
        runCatching { com.xeno.config.ConfigDatabase.ensureLocalOverrides() }
        ...
        RPCSX.instance.initialize(RPCSX.rootDirectory, "00000001", com.xeno.DeviceTier.socIdentity())
        ...
        if (!pumpsStarted) {
            ...
            Thread({ runCatching { RPCSX.instance.startMainThreadProcessor() } },
                "xeno-main-thread-processor").apply { isDaemon = true }.start()
            Thread({ runCatching { RPCSX.instance.processCompilationQueue() } },
                "xeno-compilation-queue").apply { isDaemon = true }.start()
        }
    }
     ...
     *
     * XENO signals "boot the BIOS" by passing an empty path (PCSX2 boots to the
    ...
    fun boot(path: String): Boolean {
        val target = if (path.isNotEmpty()) path else {
            ...
            if (!vsh.isFile) {
                ...
                android.util.Log.e("XENO", "XMB requested but ${'$'}{vsh.absolutePath} is missing - install firmware first")
                return false
            }
            vsh.absolutePath
        }
        ...
        appContext?.let { com.xeno.Ps3PatchRepo.ensureBundledPatches(it) }
        ...
        appContext?.let { com.xeno.OverlayIcons.ensureBundled(it) }
        ...
        if (result != BootResult.NoErrors && result != BootResult.AlreadyAdded) {
            ...
            android.util.Log.e("XENO", "boot failed: ${result.name} path=$target")
            return false
        }
        lastBootError = null
        ...
        if (RPCSX.getState() == EmulatorState.Stopped) {
            ...
            android.util.Log.e("XENO", "boot returned NoErrors but state is still Stopped; the VM never started")
            stopRumblePump()
            stopSixaxis()
            return false
        }
    ...
    /**
     * XENO's generic (section, key) setter -- THE settings entry point.
     *
     ...
     * silently dropped: each logs once to XENO-Unsupported, so a control that
     ...
     *
     * Auto-save-on-exit, auto-load-on-boot and the interval auto-save were XENO shims that
     ...
     * xeno_slot_dir builds it natively.
     */
    @JvmStatic
    fun slotFilePath(slot: Int): String? = runCatching {
        val title = RPCSX.instance.getTitleId().takeIf { it.isNotEmpty() } ?: return null
        val root = com.xeno.runtime.MainActivityRuntime.systemDirPosix()
            ?: com.xeno.runtime.MainActivityRuntime.instance
                ?.applicationContext?.getExternalFilesDir(null)?.absolutePath
            ?: return null
        java.io.File(root, "config/savestates/$title/xeno_slots/slot$slot.SAVESTAT.zst").absolutePath
    }.getOrNull()
    ...
    fun thumbnailForSlot(slot: Int): ByteArray? = runCatching {
        ...
        val root = com.xeno.runtime.MainActivityRuntime.systemDirPosix()
            ?: com.xeno.runtime.MainActivityRuntime.instance
                ?.applicationContext?.getExternalFilesDir(null)?.absolutePath
            ?: return null
        val file = java.io.File(root, "config/savestates/$title/xeno_slots/slot$slot.thumb")
    ...
    /**
     * XENO pad button indices -> PS3 CELL_PAD bits.
     *
     * XENO's indices come from PCSX2's pad enum. The face buttons do NOT line
     ...
     */
    /**
     * XENO button codes -> PS3 CELL_PAD bits.
     *
    ...
    private val transitionScheduler by lazy {
        java.util.concurrent.Executors.newSingleThreadScheduledExecutor { r ->
            Thread(r, "xeno-pad-pacing").apply { isDaemon = true }
        }
    }
     ...
     *  inherited from the XENO UI port, so the settings toggle did nothing. */
    fun setAdpfEnabled(enabled: Boolean) {
        val ctx = com.xeno.runtime.MainActivityRuntime.instance?.applicationContext ?: return
    ...
    @JvmStatic
    fun log(msg: String) { android.util.Log.i("XENO", msg) }

    // ---------------------------------------------------------------
    // Java-side helpers XENO's UI expects
    // ---------------------------------------------------------------
    ...
    private class RumbleTarget(
        ...
        private val usb: com.xeno.input.UsbRumble.Pad? = null,
    ) {
    ...
    private fun logRumble(msg: String) = android.util.Log.i("XENORumble", msg)
    ...
    private fun vibrator(port: Int = 0): RumbleTarget? {
        ...
        if (mode != PadRouter.RumbleMode.DEVICE) {
            ...
            // hardware itself.
            com.xeno.input.UsbRumble.padFor(dev)?.let { return RumbleTarget(it.label, usb = it) }
    ...
    private var sixaxis: com.xeno.input.Sixaxis? = null
    ...
    fun startSixaxis() {
        ...
        stopSixaxis()
        val feed = com.xeno.input.Sixaxis(ctx)
