 ...
 * (`overlay0_desc0 = ...`), but XENO has its own touch layout with its own editor, so honouring
...
object OverlayRepo {
     ...
     *  which XENO has its own editor for, so they are deliberately not listed. */
