...
data class OnboardingUiState(
    ...
    // stay only because other XENO screens still read them.
    ...
    // leaves XENO -- same shape as XENO's multi-BIOS picker.
...
class OnboardingViewModel(application: Application) : AndroidViewModel(application) {
     ...
     * exactly like XENO's BIOS import. Choosing which firmware to install then
            ...
            state.value = state.value.copy(
                error = if (corePresent) {
                    "The emulator core did not load, so firmware cannot be " +
                        "installed. Reinstall XENO and try again."
                } else {
    ...
    // dev_flash, and the XENO gate here asked for a .bin that does not exist
    ...
    /**
     * Winlator-style core fetch: download libxeno-core.so from the latest GitHub
