...
import com.xeno.MaliGpuInfo
import com.xeno.MaliOptimizer
...
fun PerformanceTab(state: MutableState<Settings>) {
    ...
    Column(
        ...
        // ---- Mali GPU auto-tune (XENO Mali) ----------------------------------
        // On Mali-powered devices (MediaTek Dimensity/Exynos, virtually every
        // Samsung phone since Exynos 9810, and most $150-350 budget phones) this
        // row is where the emulator's XENO Mali backend earns its keep: it knows
        // the exact GPU (Mali-G710? G715? G925?) and applies the preset tuned for
        // that tier. False positives are impossible here — the row renders
        // literally nothing unless a Mali renderer is the ACTIVE one.
        val maliProfile = remember { MaliGpuInfo.profile() }
        if (maliProfile != null && com.xeno.runtime.MainActivityRuntime.surface.value != null) {
            SettingsLabel(str("perf.mali.preset.label"))
            Text(
                maliProfile.description,
            ...
            ButtonsRow(listOf(
                ButtonInfo(
                    ...
                    backingField = com.xeno.config.Settings::applyTo is Unit,
                    onClick = {
                        // Take the Mali Optimizer's recommended settings, merge them
                        // on top of the user's CURRENT per-game value (so nothing the
                        // user set is lost), and persist through the same in-game
                        // overlay path every other row in this tab uses. Live-applies
                        // via saveSettings -> InGameOverlay -> MainActivityFeedback.
                        val rec = MaliOptimizer.getRecommendedSettings(
                            maliProfile,
                            com.xeno.runtime.MainActivityRuntime.prefsState.value,
                        )
        ...
        run {
            ...
            SegmentedRow(
                ...
                onChange = {
                    ...
                    com.xeno.runtime.MainActivityRuntime.surface.value?.applyOutputScale()
                },
            )
        }
        SettingsDivider()
        ...
        run {
            val sustained = remember { androidx.compose.runtime.mutableStateOf(com.xeno.runtime.MainActivityRuntime.prefs.getBoolean("ui.sustainedPerf", false)) }
            SegmentedRow(
                ...
                onChange = {
                    val on = it == 1
                    sustained.value = on
                    com.xeno.runtime.MainActivityRuntime.prefs.edit {
                        putBoolean(
                            "ui.sustainedPerf",
                            on
                        )
                    ...
                    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                        runCatching {
                            (com.xeno.runtime.MainActivityRuntime.surface.value?.context as? android.app.Activity)
        ...
        run {
            val adpf = remember { androidx.compose.runtime.mutableStateOf(com.xeno.runtime.MainActivityRuntime.prefs.getBoolean("ui.adpf", false)) }
            SegmentedRow(
                ...
                onChange = {
                    val on = it == 1
                    adpf.value = on
                    com.xeno.runtime.MainActivityRuntime.prefs.edit { putBoolean("ui.adpf", on) }
                    runCatching { com.xeno.NativeApp.setAdpfEnabled(on) }
                },
            )
        }
        SettingsDivider()
        ...
        // Absent from the play build, where libxeno_lsfg.so is not in the bundle. The core
