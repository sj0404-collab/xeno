...
fun OverlayTab(state: MutableState<Settings>) {
    ...
    Column(
        ...
        val ffToasts = remember { mutableStateOf(com.xeno.runtime.MainActivityRuntime.prefs.getBoolean("ui.hotkeyToasts", true)) }
        ToggleRow(str("overlay.toggle.fastForwardPopups"), ffToasts.value) {
            ffToasts.value = it
            com.xeno.runtime.MainActivityRuntime.prefs.edit { putBoolean("ui.hotkeyToasts", it) }
        }
        SettingsDivider()
        ...
        ToggleRow(
            ...
            com.xeno.Thermals.osdEnabled.value,
            description = str("overlay.toggle.temps.description"),
        ) {
            com.xeno.Thermals.setOsdEnabled(ctx, it)
        }
        ...
        // syscalls.
        if (com.xeno.Thermals.osdEnabled.value) {
            IntSliderRow(
                label = str("overlay.tempInterval.label"),
                value = com.xeno.Thermals.intervalSec.value,
                min = 1,
                max = 5,
                valueFormatter = { "${it}s" },
                onChange = { com.xeno.Thermals.setIntervalSec(it) },
            )
        }
    }
}
