 ...
 * input-mode latch.
 *
 * Three storage tiers in MainActivityRuntime.prefs (single "XENO" SharedPreferences):
...
object TouchControls {
     ...
     *  corner: XENO parks its shoulders far higher than NetherSX2 does (R2 at y=0.10 vs ~0.39),
    ...
    fun reapplyPressureToHeldButtons() {
        ...
        for ((port, keys) in snapshot) {
            for (kc in keys) {
                runCatching {
                    com.xeno.NativeApp.setPadButtonForPort(port, kc, pressureRangeFor(kc), true)
                }
            }
        }
    }
        ...
        if (gameIsRunning()) {
            ...
            if (serial != null) {
                ...
                if (crc != null) {
                    ...
                    println(
                        "@@XENO_TOUCH@@ refusing to save in-game layout to global " +
