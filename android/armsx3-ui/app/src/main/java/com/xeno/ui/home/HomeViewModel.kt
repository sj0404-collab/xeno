...
class HomeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun load(romDirectories: List<String>, nativeReady: Boolean) {
        ...
        if (!loaded) {
            ...
            android.util.Log.i("XENO-Scan", "load(first): dirs=$romDirectories nativeReady=$nativeReady cachedKey=${cached.key} newKey=${repository.cacheKey(romDirectories)} cachedGames=${cached.games.size} pending=$pendingInitialScan")
            ...
            if (nativeReady) com.xeno.RaLibrary.onLibraryLoaded(state.value.allGames)
            if (nativeReady && pendingInitialScan) refresh()
        } else {
            android.util.Log.i("XENO-Scan", "load(again): dirs=${romDirectories.size} nativeReady=$nativeReady pending=$pendingInitialScan")
            if (nativeReady && pendingInitialScan) refresh()
        }
    }
    ...
    fun refresh() {
        ...
        scanJob = scope.launch {
            ...
            result.onSuccess { games ->
                ...
                com.xeno.RaLibrary.onLibraryLoaded(games)
            }.onFailure { failure ->
                pendingInitialScan = false
                state.value = state.value.copy(
                    scanning = false,
    ...
    fun launch(game: GameInfo) {
        ...
        if (game.locked) {
            com.xeno.LicencePrompt.ask(game)
            return
        }
    ...
    fun setHidden(game: GameInfo, value: Boolean) {
        com.xeno.HiddenGames.setHidden(game, value)
        state.value = buildState(state.value)
    }
    ...
    fun refreshCategories() {
        ...
        val stillExists = active == null || active in com.xeno.GameCategories.names()
    ...
    fun setShowHidden(value: Boolean) {
        com.xeno.HiddenGames.setShowHidden(value)
        state.value = buildState(state.value)
    }
    ...
    private fun buildState(base: HomeUiState): HomeUiState {
        ...
        val forceEn = com.xeno.EnglishTitles.enabled.value
        val filtered = base.allGames.filter { game ->
            ...
            (base.categoryFilter == null ||
                game.settingsKey?.let { it in com.xeno.GameCategories.membersOf(base.categoryFilter) } == true) &&
                (com.xeno.HiddenGames.showHidden.value || !com.xeno.HiddenGames.isHidden(game)) &&
                (query.isBlank() ||
