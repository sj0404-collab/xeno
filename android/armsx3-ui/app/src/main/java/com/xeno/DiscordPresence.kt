...
import com.xeno.discord.DiscordIpc

/**
 * Discord rich presence, and which friends are in XENO right now.
 *
 ...
 * while XENO is open and not otherwise. Background delivery would mean push infrastructure, which
 ...
 */
/**
 * A Discord friend who has XENO open.
 *
...
data class DiscordFriend(
    ...
    /** In XENO but not in a game. Distinct from being in a game we could not name. */
...
object DiscordPresence {
    ...
    // ---- helper-process link -------------------------------------------------------------
    //
    // The Discord SDK runs in :discord, not here. That is a licensing boundary: XENO is GPL-3.0+
    ...
    private fun bindHelper() {
        ...
        val intent = Intent(ctx, com.xeno.discord.DiscordService::class.java)
        bound = runCatching {
    ...
    private fun raPresence(): String {
        ...
        val presence = runCatching { com.xeno.NativeApp.getRichPresence().orEmpty() }
            .getOrDefault("").trim()

        val badge = runCatching {
            val items = com.xeno.ui.achievements.parseAchievementItems(
                com.xeno.NativeApp.getAchievementsJSON().orEmpty(),
            )
            if (items.isEmpty()) return@runCatching ""
            val hardcore = com.xeno.NativeApp.isHardcoreMode()
     ...
     *
     * The SDK used to need XENO's Activity to launch sign-in. It now lives in :discord and uses
