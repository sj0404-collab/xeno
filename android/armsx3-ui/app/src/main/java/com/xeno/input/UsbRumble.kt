 ...
 */
object UsbRumble {
    private const val TAG = "XENORumble"
    ...
    private const val ACTION_PERMISSION = "com.xeno.action.USB_RUMBLE_PERMISSION"
    ...
    fun loadTakeover() {
        takeover = runCatching {
            com.xeno.runtime.MainActivityRuntime.prefs.getBoolean(KEY_TAKEOVER, false)
        }.getOrDefault(false)
    }
    ...
    fun setTakeover(on: Boolean) {
        takeover = on
        runCatching {
            com.xeno.runtime.MainActivityRuntime.prefs.edit().putBoolean(KEY_TAKEOVER, on).apply()
        }
        val open = pad
        if (on) {
