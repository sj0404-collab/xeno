...
/**
 * Cloud save + cloud game sync for XENO.
 *
...
object CloudSync {
    ...
    data class Config(
        /** WebDAV base URL, e.g. https://host/dav/xeno/ */
     ...
     *  [com.xeno.runtime.MainActivityRuntime] so the launch diagnostics never leak
    ...
    fun load() {
        val prefs = runCatching { com.xeno.runtime.MainActivityRuntime.prefs }
            .getOrNull() ?: return
    ...
    fun updateAutoPush(enabled: Boolean) {
        autoPush = enabled
        runCatching { com.xeno.runtime.MainActivityRuntime.prefs }
    ...
    suspend fun streamGameUrl(remoteName: String): String? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent", "XENO-CloudSync/1.0")
