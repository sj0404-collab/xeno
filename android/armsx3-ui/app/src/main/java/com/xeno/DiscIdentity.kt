...
object DiscIdentity {
    ...
    suspend fun resolve(uri: android.net.Uri, serial: String?): String? =
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            runCatching {
                ...
                if (wanted != null &&
                    com.xeno.NativeApp.getGameSerial()?.takeIf { it.isNotBlank() } == wanted
                ) {
                    com.xeno.NativeApp.getGameCRC()
    ...
    /** Internal-use path the native side expects for [uri]. Also used by [com.xeno.RaLibrary]
